...
# Responses to reviewer comments on repostitioning paper submitted to:
...
for (i in 1:nrow(sideeffects)){
  if(str_detect(sideeffects[i,2],"\\([^()]+\\)")){   # regex
...
# and obtain the side effects for each one. NOTE: spellings and case sensitivity is important!!
